...
const SUBJECT_COLORS = {
  Biology:      "emerald", Mathematics: "blue", English: "amber",
  Physics:      "purple", Chemistry: "red", History: "green",
  Other:        "gray",
};

const RESOURCE_TYPE_MAP = [
  { label: "PDF Document",  type: "PDF",   icon: <FaFilePdf />, color: "red" },
  { label: "Word Document", type: "PDF",   icon: <FaFileWord />, color: "blue" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-900 to-gray-800 text-gray-200 p-6">
      ...
      <main className="max-w-6xl mx-auto p-4">
        ...
        <section className="relative overflow-hidden bg-gradient-to-r from-emerald-900/30 to-teal-900/30 backdrop-blur-sm border border-emerald-500/20 rounded-2xl p-8 mb-6">
          ...
          <div className="relative flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="text-gray-400 text-sm">Upload and manage educational materials for your students</p>
            </div>
        ...
        <section className="flex flex-wrap gap-3 mb-6 items-center">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input type="text" placeholder="Search resources..."
              ...
              className="w-full bg-gray-800/50 border border-gray-700 text-gray-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 placeholder-gray-500 transition-all" />
          </div>
          <select value={filterForm} onChange={(e) => setFilterForm(e.target.value)}
            className="bg-gray-800/50 border border-gray-700 text-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all">
        ...
        {!loading && filtered.length === 0 ? (
          <div className="bg-gray-800/30 border border-gray-700 rounded-2xl p-16 text-center">
          ...
          <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
            {filtered.map((book) => {
              ...
              return (
                <div key={book.id}
                  className="group bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl overflow-hidden hover:border-gray-600 hover:translate-y-[-2px] transition-all duration-300">
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="font-semibold text-gray-200 mb-2 leading-snug text-lg">
                      {book.title}
                    </h3>
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-400 mb-3 pb-3 border-b border-gray-700">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setDeleteTarget(book)}
                        className="flex-1 bg-gray-700 hover:bg-gray-600 text-gray-200 text-sm font-medium py-2 rounded-lg transition-all flex items-center justify-center gap-2">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 modal-animation">
          <div className="bg-gray-800 border border-gray-700 rounded-2xl w-full max-w-lg p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div
              ...
              className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer mb-5 transition-all ${
                dragOver ? "border-emerald-500 bg-emerald-500/10" : "border-gray-600 bg-gray-900/50"
              }`}>
              ...
              {form.file ? (
                <div>
                  ...
                  <p className="text-sm text-gray-200 font-medium">{form.file.name}</p>
                  <p className="text-xs text-gray-400 mt-1">{(form.file.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
              ) : (
                <div>
                  <IoCloudUploadOutline size={48} className="mx-auto mb-2 text-gray-400" />
                  <p className="text-sm text-gray-300">Drag & drop or click to browse</p>
                  <p className="text-xs text-gray-500 mt-1">PDF, DOCX, MP4, Images</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3 mb-5">
              <input type="text" placeholder="Resource Title" value={form.title}
                ...
                className="w-full bg-gray-900/50 border border-gray-700 text-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 placeholder-gray-500 transition-all" />

              <textarea placeholder="Description (optional)" value={form.description} rows={2}
                onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                className="w-full bg-gray-900/50 border border-gray-700 text-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 placeholder-gray-500 resize-none" />

              <div className="grid grid-cols-2 gap-3">
                <select value={form.categoryId}
                  onChange={(e) => setForm((f) => ({ ...f, categoryId: e.target.value }))}
                  className="bg-gray-900/50 border border-gray-700 text-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all">
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowModal(false)}
                className="flex-1 bg-gray-700 hover:bg-gray-600 text-gray-200 font-medium py-2.5 rounded-xl transition-all text-sm">
                Cancel
              </button>
              <button onClick={handleSubmit} disabled={uploading}
                className="flex-1 bg-gradient-to-r from-emerald-600 to-emerald-500 text-white font-medium py-2.5 rounded-xl hover:from-emerald-500 hover:to-emerald-600 transition-all text-sm disabled:opacity-50 shadow-lg shadow-emerald-500/25">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 modal-animation">
          <div className="bg-gray-800 border border-red-500/50 rounded-2xl w-full max-w-sm p-6 shadow-2xl">
            <MdDeleteForever size={48} className="mx-auto mb-3 text-red-400" />
            <h2 className="text-lg font-bold text-gray-200 text-center mb-2">Delete Resource?</h2>
            <p className="text-sm text-gray-400 text-center mb-6">
              Are you sure you want to delete <span className="font-semibold text-gray-200">"{deleteTarget.title}"</span>?
            </p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTarget(null)}
                className="flex-1 bg-gray-700 hover:bg-gray-600 text-gray-200 font-medium py-2 rounded-xl transition-all">
                Cancel
              </button>
              <button onClick={handleRequestDelete}
                className="flex-1 bg-red-500/20 border border-red-500/50 text-red-400 font-medium py-2 rounded-xl hover:bg-red-500/30 transition-all">
                Delete
              </button>
            </div>
          </div>
